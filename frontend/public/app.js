...
function initGlobe() {
  if (typeof Cesium === "undefined") {
    document.getElementById("globe").innerHTML = "<div style=\"padding:18px;color:#fff\">Cesium failed to load. Check network/CSP settings.</div>";
    return;
  }
...
function renderLiveStocks(companies) {
  const tbody = document.querySelector("#live-stocks-table tbody");
  tbody.innerHTML = companies.slice(0, 15).map((c) => `<tr><td>${c.symbol}</td><td>${c.name}</td><td>${c.price ? usd(c.price) : "N/A"}</td><td>${c.change_percent ? `${c.change_percent.toFixed(2)}%` : "N/A"}</td></tr>`).join("");
}

function renderDataCatalog(rows) {
  const box = document.getElementById("data-catalog");
  box.innerHTML = rows.map((r) => `<div class="catalog-item"><b>${r.dataset}</b><br/><span>${r.records} records</span><br/><small>${r.keys.join(", ")}</small></div>`).join("");
}
...
async function refreshLiveMap() {
  ...
  renderLiveStocks(companies.items);
